...
async def get_content_by_topic(request, topic: str, db: AsyncSession = Depends(get_db)):
...
@limiter.limit("5/minute")
async def search_content(request, query: str, db: AsyncSession = Depends(get_db)):
...
@limiter.limit("20/minute")
async def get_module_progress(user_id: int, module_id: str, db: AsyncSession = Depends(get_db)):
...
@limiter.limit("10/minute")
async def get_user_learning_path(user_id: int, db: AsyncSession = Depends(get_db)):
