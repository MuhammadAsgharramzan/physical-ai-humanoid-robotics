...
function SignupPageContent() {
  const { signup, error: authError, isLoading } = useUser();
  ...
  const handleSubmit = async (e) => {
    ...
    if (!username.trim() || !email.trim() || !password) {
      setFormError(translate({ id: 'authentication.validation.all_fields_required', message: 'All fields are required' }));
      return;
    }

    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(email)) {
      setFormError(translate({ id: 'authentication.validation.valid_email_required', message: 'Please enter a valid email' }));
      return;
    }

    if (password.length < 6) {
      setFormError(translate({ id: 'authentication.validation.password_min_length', message: 'Password must be at least 6 characters' }));
      return;
    }

    if (password !== confirmPassword) {
      setFormError(translate({ id: 'authentication.validation.passwords_match', message: 'Passwords do not match' }));
      return;
    }

    setFormError('');

    // Call the signup function from UserContext
    const result = await signup(username, email, password);
    if (result.success) {
      // Successful signup - redirect to home
      window.location.href = '/';
    } else {
      setFormError(result.error || translate({ id: 'authentication.error.signup_failed', message: 'Sign up failed' }));
    }
  };
  ...
  const handleSocialLogin = async (provider) => {
    ...
    try {
      ...
      setFormError(translate({
        id: 'authentication.error.social_signup_failed',
  ...
  return (
    <div className="auth-page-container">
      <div className="auth-form-wrapper">
        <h1>{translate({ id: 'authentication.signup.header', message: 'Create Your Account' })}</h1>
        ...
        <form onSubmit={handleSubmit} className="auth-form">
          <div className="form-group">
            <label htmlFor="signup-username">{translate({ id: 'authentication.field.username', message: 'Username' })}</label>
            <input
              id="signup-username"
              type="text"
              value={username}
              onChange={(e) => setUsername(e.target.value)}
              placeholder={translate({ id: 'authentication.placeholder.choose_username', message: 'Choose a username' })}
              required
            />
          </div>

          <div className="form-group">
            <label htmlFor="signup-email">{translate({ id: 'authentication.field.email', message: 'Email' })}</label>
            <input
              id="signup-email"
              type="email"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              placeholder={translate({ id: 'authentication.placeholder.email', message: 'Enter your email' })}
              required
            />
          </div>

          <div className="form-group">
            <label htmlFor="signup-password">{translate({ id: 'authentication.field.password', message: 'Password' })}</label>
            <input
              id="signup-password"
              type="password"
              value={password}
              onChange={(e) => setPassword(e.target.value)}
              placeholder={translate({ id: 'authentication.placeholder.password', message: 'Create a password (min 6 characters)' })}
              required
            />
          </div>

          <div className="form-group">
            <label htmlFor="signup-confirm-password">{translate({ id: 'authentication.field.confirm_password', message: 'Confirm Password' })}</label>
            <input
              id="signup-confirm-password"
              type="password"
              value={confirmPassword}
              onChange={(e) => setConfirmPassword(e.target.value)}
              placeholder={translate({ id: 'authentication.placeholder.confirm_password', message: 'Confirm your password' })}
              required
            />
          </div>
          ...
          <button type="submit" className="auth-submit-button" disabled={isLoading}>
            {isLoading
              ? translate({ id: 'authentication.button.creating_account', message: 'Creating account...' })
              : translate({ id: 'authentication.button.signup', message: 'Sign Up' })}
          </button>
        </form>

        <div className="auth-page-footer">
          <p>
            {translate({ id: 'authentication.footer.already_have_account', message: 'Already have an account?' })}{' '}
            <a href="/login">{translate({ id: 'authentication.footer.login_link', message: 'Login' })}</a>
          </p>
        </div>
      </div>
    </div>
...
export default function SignupPage() {
  return (
    <Layout
      title={translate({ id: 'authentication.signup.title', message: 'Sign Up' })}
      description={translate({ id: 'authentication.signup.description', message: 'Create a new account' })}>
      <SignupPageContent />
    </Layout>
  );
}